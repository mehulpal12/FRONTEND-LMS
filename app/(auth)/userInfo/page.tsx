...
export default function ProfilePage() {
  ...
  return (
    <>
      <Navbar />
     <main className="min-h-screen bg-background p-6 lg:p-12 font-sans">
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div 
          ...
          className="bg-background rounded-[32px] p-8 border border-[#C1C6D6]/20 shadow-sm flex flex-col md:flex-row items-center gap-8"
        >
          <div className="relative group">
            ...
            <button className="absolute -bottom-2 -right-2 bg-background p-2 rounded-xl shadow-lg border border-gray-100 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-col md:flex-row md:items-center gap-3 mb-2">
              <h1 className="text-3xl font-black text-primary tracking-tighter">{userData?.name}</h1>
              <span className="px-3 py-1 bg-background text-primary text-[10px] font-black uppercase tracking-widest rounded-full w-fit mx-auto md:mx-0">
                {userData?.role}
              </span>
            </div>
            <div className="flex flex-wrap justify-center md:justify-start gap-4 text-primary text-sm font-medium">
        ...
        <div className="mt-12">
          <h2 className="text-2xl font-black text-primary mb-6 tracking-tight">Active Curriculum</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {userData?.enrollments.map((item, idx) => (
              <motion.div 
                key={idx}
                whileHover={{ y: -5 }}
                className="bg-background p-6 rounded-[24px] border border-[#C1C6D6]/20 flex gap-4 items-center"
              >
                <img src={item.course.thumbnail || "/api/placeholder/100/100"} className="w-20 h-20 rounded-2xl object-cover" alt="Course" />
                <div className="flex-1">
                  <h3 className="font-bold text-primary leading-tight">{item.course.title}</h3>
...
function StatCard({ icon, label, value }: { icon: any, label: string, value: string | number }) {
  return (
    <div className="bg-background p-6 rounded-[28px] border border-[#C1C6D6]/20 shadow-sm">
...
function ProfileSkeleton() {
  return (
    <div className="min-h-screen bg-background p-12 flex justify-center">
      <div className="w-full max-w-5xl animate-pulse bg-background rounded-[32px] h-[300px]" />
    </div>
  );
}